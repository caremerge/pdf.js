...
describe("Writer", function () {
  describe("Incremental update", function () {
    it("should update a file with new objects", function (done) {
      var originalData = new Uint8Array();
      var newRefs = [{
      ...
      var data = (0, _writer.incrementalUpdate)({
  ...
  describe("writeDict", function () {
    it("should write a Dict", function (done) {
      ...
      var gdict = new _primitives.Dict(null);
      gdict.set("H", 123.00001);
      var string = "a stream";
      var stream = new _stream.StringStream(string);
      ...
      var expected = "<< /A /B /B 123 456 R /C 789 /D (hello world) " + "/E (\\(hello\\\\world\\)) /F [1.23 4.5 6] " + "/G << /H 123 /I << /Length 8>> stream\n" + "a stream\n" + "endstream\n>>>>";
    ...
    it("should write a Dict in escaping PDF names", function (done) {
      ...
      var buffer = [];
      (0, _writer.writeDict)(dict, buffer, null);
      var expected = "<< /#feA#23 /hello /B /#23hello /C /he#fello#ff>>";
      expect(buffer.join("")).toEqual(expected);
      done();
    });
  });
